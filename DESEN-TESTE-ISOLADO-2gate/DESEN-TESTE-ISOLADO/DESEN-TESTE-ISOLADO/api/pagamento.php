...
// Nome e preço saem do catálogo interno (api/produtos.php). O front manda só o
// código da etapa; product_name e valor vindos do POST são ignorados de
// propósito, para ninguém escolher o que paga montando a URL na mão.
$ETAPA   = catalogo_etapa(isset($_POST['etapa']) ? $_POST['etapa'] : '', 'front');
$PRODUTO = $ETAPA['nome'];
...
$sck          = isset($_POST['sck']) ? $_POST['sck'] : null;
$gclid        = isset($_POST['gclid']) ? $_POST['gclid'] : null;
$fbclid       = isset($_POST['fbclid']) ? $_POST['fbclid'] : null;
$ttclid       = isset($_POST['ttclid']) ? $_POST['ttclid'] : null;
$leadId       = isset($_POST['leadId']) ? $_POST['leadId'] : null;
...
foreach (array('utm_source','utm_campaign','utm_medium','utm_content','utm_term','src','sck','gclid','fbclid','ttclid','leadId') as $k) {
    if (!empty($$k)) $metaObj[$k] = $$k;
}

$externalRef = 'serasa-' . time() . '-' . rand(1000, 9999);
